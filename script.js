...
const EDGE_SHEET_URL =
  "https://script.google.com/macros/s/AKfycbzr4gSGOcfiaHUWuyz-SwAl8ZSmWSmQRTHqOe-W3IfYIEe5Z1rHzVr7OXLhyroad_8N/exec";
...
// --- File upload elements (Resume + DL)
...
const dlUploadBtn = document.getElementById("dlUploadBtn");
...
/* ---------------- File Upload Helpers & Handlers (INSERTED - Option B) ---------------- */

// Enhancement settings
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10 MB

// allowed mime types / extensions for each field (basic client-side check)
const ALLOWED = {
  resume: [
    "application/pdf",
    "application/msword",
...
// build suggested filename: TYPE_CandidateName_Title_TIMESTAMP.ext
function buildSuggestedFilename({ type, candidateName, title, originalName }) {
  const ext = (originalName || "").split(".").pop() || "";
  const namePart = sanitizeForFilename(candidateName || "no_name");
  const titlePart = sanitizeForFilename(title || "no_title");
  const ts = Date.now();
  return `${type}_${namePart}_${titlePart}_${ts}.${ext}`;
}

/**
 * uploadFile(file, fileType, suggestedName)
 * - fileType: 'resume' or 'dl' (used by server)
 * - suggestedName: suggested filename to be used server-side (Apps Script may use it)
 * returns: fileUrl (string) on success
 */
async function uploadFile(file, fileType, suggestedName) {
  const candidateName =
    document.getElementById("candidateName")?.value || "Unknown";
  const title =
  ...
  const resp = await fetch(EDGE_SHEET_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
if (resumeUploadBtn) {
  resumeUploadBtn.addEventListener("click", async (evt) => {
    evt.preventDefault();
    const file = resumeFileInput?.files?.[0];
    if (!file) return alert("Please select a resume file before uploading.");

    // set state
    isResumeUploading = true;
    resumeUploadBtn.disabled = true;
    const origText = resumeUploadBtn.textContent;
    resumeUploadBtn.textContent = "Uploading...";

    try {
      const suggested = buildSuggestedFilename({
        type: "RESUME",
        candidateName: document.getElementById("candidateName")?.value || "no_name",
        title: document.getElementById("title")?.value || "no_title",
        originalName: file.name,
      });

      const fileUrl = await uploadFile(file, "resume", suggested);

      // success
      resumeField.value = fileUrl || "";
      resumeFileName.textContent = `Uploaded: ${file.name}`;
      resumeUploadBtn.textContent = "Uploaded!";
    } catch (err) {
      alert("Resume upload failed: " + (err.message || err));
      console.error("Resume upload error:", err);
      resumeUploadBtn.textContent = origText;
    } finally {
...
// DL upload button
if (dlUploadBtn) {
  dlUploadBtn.addEventListener("click", async (evt) => {
    evt.preventDefault();
    const file = dlFileInput?.files?.[0];
    if (!file) return alert("Please select a DL file before uploading.");

    // set state
    isDLUploading = true;
    dlUploadBtn.disabled = true;
    const origText = dlUploadBtn.textContent;
    dlUploadBtn.textContent = "Uploading...";
    ...
    try {
      ...
      const fileUrl = await uploadFile(file, "dl", suggested);

      dlField.value = fileUrl || "";
      dlFileName.textContent = `Uploaded: ${file.name}`;
      dlUploadBtn.textContent = "Uploaded!";
    } catch (err) {
      alert("DL upload failed: " + (err.message || err));
      console.error("DL upload error:", err);
      dlUploadBtn.textContent = origText;
    } finally {
      isDLUploading = false;
...
/* ---------------- Candidate submit handler (unchanged location, but now after upload handlers) ---------------- */
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      if (editingId) {
        ...
        const nextId = maxData?.length ? Number(maxData[0].Unique) + 1 : 1;

        const newRecord = { Unique: nextId, ...record };
